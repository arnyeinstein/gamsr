...
import_gdxfile <- function(filename, selection = NULL) {
  ...
  for (i in 1:length(parinfo[, 1])) {
    ...
    if (parinfo[i, "type"] == "parameter" & parinfo[i, "dim"] == 0) {
      text[i] <- paste(parinfo[i, "name"], " <- ", "gdxrrw::rgdx.scalar('",
                       filename, "', '", parinfo[i, "name"], "'", ")",
                       sep = "")
      texta <- paste("attributes(", parinfo[i, "name"],
                     ")$type <- 'scalar'", sep = "")
    }
    ...
    if (parinfo[i, "type"] == "variable") {
      text[i] <- paste(parinfo[i, "name"], " <- ", "gdxrrw::rgdx(\"",
                       filename, "\", list(name =\"", parinfo[i, "name"],
                       "\", field = \"all\"))", sep = "")
      texta <- paste("attributes(", parinfo[i, "name"],
                     ")$type <- 'variable'", "\n", parinfo[i, "name"],
    ...
    textl <- paste("l.all[[", i, "]] <- list(name ='", parinfo[i, "name"],
                   sep = "")
    textl <- paste(textl, "', data = ", parinfo[i, "name"], sep = "")
    textl <- paste(textl, ", description = '", parinfo[i, "text"],
                   sep = "")
    textl <- paste(textl, "'", ", type = '", parinfo[i, "type"], "')",
                   sep = "")
    textn <- paste( "names(l.all)[", i, "] ='", parinfo[i, "name"], "'",
                    sep = "")
    # Replace * in parameters, sets or variables with "index"
    textr[i]  <- paste("names(", parinfo[i, "name"], ") <- gsub(x = names(",parinfo[i, "name"], "), pattern = '*', replacement = 'index', fixed=TRUE)", sep = "")
    text[i] <- paste(text[i], texta, textl, textn, textr[i], sep = "\n ")
  }
  ## End for loop
